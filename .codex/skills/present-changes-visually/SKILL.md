...
imperative subject and include enough detail in the body to explain what changed
and why.
...
4. Run the bundled generator from the repository root. Prefer `python3`; if it
   is not available on Windows, call `load_workspace_dependencies` and use the
   returned bundled Python executable.
